import type { Locale } from "./config";

const ui = {
  en: {
    // Nav
    "nav.home": "Home",
    "nav.categories": "Categories",
    "nav.create": "Create a quiz",
    "nav.profile": "My Profile",
    "nav.login": "Log in",
    "nav.signup": "Sign up",
    "auth.logout": "Log out",
    "nav.account": "My account",
    "nav.search": "Search a quiz, a category...",
    "nav.ranking": "Ranking",

    // Home
    "home.hero.title": "The #1 quiz platform",
    "home.hero.subtitle": "Test your knowledge with fun and interactive quizzes!",
    "home.hero.cta": "Start a quiz",
    "home.hero.create": "Create a quiz",
    "home.stats.quizzes": "quizzes",
    "home.stats.questions": "questions",
    "home.stats.plays": "games played",
    "home.popular": "Most popular",
    "home.new": "New quizzes",
    "home.categories": "All categories",
    "home.seeAll": "See all",
    "home.trending": "Trending",
    "home.gameTypes": "Game modes",
    "home.questionOfDay": "Question of the day",
    "home.faq.title": "Frequently asked questions",
    "home.member.title": "Become a member",
    "home.member.cta": "Create my account",
    "home.member.free": "Free, no commitment",
    "home.member.scores": "Save your scores",
    "home.member.xp": "Earn XP points",
    "home.member.compare": "Compare with others",
    "home.member.text": "An account keeps your scores and your XP, and lets you compare yourself with your friends.",

    // Quiz
    "quiz.questions": "questions",
    "quiz.question": "Question",
    "quiz.plays": "games played",
    "quiz.difficulty": "Difficulty",
    "quiz.start": "Start quiz",
    "quiz.next": "Next",
    "quiz.finish": "See results",
    "quiz.score": "Your score",
    "quiz.perfect": "Perfect score!",
    "quiz.great": "Great job!",
    "quiz.good": "Nice try!",
    "quiz.retry": "Try again",
    "quiz.share": "Share",
    "quiz.recommended": "Recommended quizzes",
    "quiz.allInCategory": "All {category} quizzes",
    "quiz.otherInCategory": "{count} other quiz(zes) in this category",
    "quiz.discoverCategory": "Discover all quizzes in this category",
    "quiz.correct": "Correct!",
    "quiz.wrong": "Wrong!",
    "quiz.explanation": "Explanation",

    // Category
    "cat.quizzes": "quiz(zes)",
    "cat.subcategories": "Subcategories",
    "cat.allQuizzes": "All quizzes",
    "cat.featured": "Featured",
    "cat.filter": "Filter",
    "cat.sort.popular": "Most popular",
    "cat.sort.new": "Newest",
    "cat.sort.all": "All",
    "cat.explore": "Explore categories",
    "cat.noQuiz": "No quizzes yet in this category.",

    // Difficulty
    "difficulty.easy": "Easy",
    "difficulty.medium": "Medium",
    "difficulty.hard": "Hard",

    // Game types
    "gameType.qcm": "Multiple Choice",
    "gameType.vrai-faux": "True or False",
    "gameType.chrono": "Chrono Challenge",
    "gameType.estimation": "Estimation",
    "gameType.duel": "Duel: who has more?",
    "gameType.ordre": "Chronological Order",

    // Sitemap
    "sitemap.title": "Sitemap",
    "sitemap.mainPages": "Main pages",
    "sitemap.allCategories": "All categories",

    // Footer
    "footer.about": "About",
    "footer.legal": "Legal notices",
    "footer.privacy": "Privacy",
    "footer.terms": "Terms",
    "footer.sitemap": "Sitemap",
    "footer.description": "Free trivia quizzes with an explanation under every answer. No account needed to play.",
    "footer.quizzesAndGames": "Quizzes & Games",
    "footer.allQuizzes": "All quizzes",
    "footer.createQuiz": "Create a quiz",
    "footer.community": "Community quizzes",
    "footer.information": "Information",
    "footer.contactUs": "Contact us",
    "footer.allRightsReserved": "All rights reserved.",

    // Breadcrumb
    "breadcrumb.home": "Home",
    "games.hubName": "Games",
    "games.hubHeading": "Mini games to play right now",
    "games.hubIntro": "Short games you can start in one tap, with no account and no setup. They draw their rounds at random, so you can come back to the same one and not play the same thing twice.",
    "games.hubMetaTitle": "Free Mini Games to Play Online | WizyQuiz",
    "games.hubMetaDescription": "Short trivia games you start in one tap, with no account. Higher or lower on countries, summits, rivers and films, with a streak to beat.",
    "games.howToPlay": "How to play",
    "games.faq": "Frequently asked questions",
    "games.moreTitle": "The other games",
    "games.moreIntro": "Every game here starts in one tap and asks nothing of you.",
    "games.seeAll": "See all the games",
    "games.vsQuizTitle": "Games and quizzes are not the same thing",
    "games.vsQuizP1": "A quiz has twenty questions, a score and an end: you play it, you get your result, and that is that. A game draws its rounds at random and stops only when you get one wrong, so the same game gives you something different every time.",
    "games.vsQuizP2": "The two go together. A quiz is the one you send to a friend to compare scores on a subject you both know. A game is the one you open while you wait for the bus.",
    "games.seeQuizzes": "See the quizzes",

    // Auth
    "auth.login": "Log in",
    "auth.signup": "Sign up",
    "auth.email": "Email address",
    "auth.password": "Password",
    "auth.confirmPassword": "Confirm password",
    "auth.username": "Username",
    "auth.loginCta": "Log in",
    "auth.signupCta": "Create my account",
    "auth.noAccount": "No account yet?",
    "auth.hasAccount": "Already have an account?",
    "auth.loginTitle": "Log In to Your WizyQuiz Account | Free Quiz Platform",
    "auth.signupTitle": "Sign Up for Free | WizyQuiz Quiz Platform",
    "auth.loginDescription": "Log in to your WizyQuiz account to save your scores, track your progress and challenge your friends.",
    "auth.signupDescription": "Create your free WizyQuiz account and start tracking your quiz scores, earning XP and competing with friends.",

    // Ranking
    "ranking.title": "Leaderboard",
    "ranking.subtitle": "Climb the ranks by earning XP from quizzes. The more you play, the higher you rise.",
    "ranking.rank": "Rank",
    "ranking.player": "Player",
    "ranking.level": "Level",
    "ranking.xp": "XP",
    "ranking.quizzes": "Quizzes",
    "ranking.allTime": "All time",
    "ranking.thisWeek": "This week",
    "ranking.thisMonth": "This month",
    "ranking.metaTitle": "Leaderboard and Rankings | WizyQuiz",
    "ranking.metaDescription": "Check the WizyQuiz leaderboard. Compete with other players, earn XP by completing quizzes and climb the ranks.",

    // Misc
    "misc.free": "Free",
    "misc.seeMore": "See more",
    "misc.language": "Language",
  },

  fr: {
    "nav.home": "Accueil",
    "nav.categories": "Catégories",
    "nav.create": "Créer un quiz",
    "nav.profile": "Mon Profil",
    "nav.login": "Se connecter",
    "nav.signup": "S'inscrire",
    "auth.logout": "Se déconnecter",
    "nav.account": "Mon compte",
    "nav.search": "Rechercher un quiz, une catégorie...",
    "nav.ranking": "Classement",

    "home.hero.title": "La plateforme de quiz n°1",
    "home.hero.subtitle": "Teste tes connaissances avec nos quiz fun et interactifs !",
    "home.hero.cta": "Lancer un quiz",
    "home.hero.create": "Créer un quiz",
    "home.stats.quizzes": "quiz",
    "home.stats.questions": "questions",
    "home.stats.plays": "parties jouées",
    "home.popular": "Les plus populaires",
    "home.new": "Nouveautés",
    "home.categories": "Toutes les catégories",
    "home.seeAll": "Voir tout",
    "home.trending": "Tendances",
    "home.gameTypes": "Modes de jeu",
    "home.questionOfDay": "Question du jour",
    "home.faq.title": "Questions fréquentes",
    "home.member.title": "Devenez membre",
    "home.member.cta": "Créer mon compte",
    "home.member.free": "Gratuit, sans engagement",
    "home.member.scores": "Enregistrez vos scores",
    "home.member.xp": "Gagnez des points XP",
    "home.member.compare": "Comparez-vous aux autres",
    "home.member.text": "Un compte garde vos scores et vos XP, et vous permet de vous comparer à vos amis.",

    "quiz.questions": "questions",
    "quiz.question": "Question",
    "quiz.plays": "parties jouées",
    "quiz.difficulty": "Difficulté",
    "quiz.start": "Lancer le quiz",
    "quiz.next": "Suivant",
    "quiz.finish": "Voir les résultats",
    "quiz.score": "Ton score",
    "quiz.perfect": "Score parfait !",
    "quiz.great": "Bravo !",
    "quiz.good": "Bien joué !",
    "quiz.retry": "Rejouer",
    "quiz.share": "Partager",
    "quiz.recommended": "Quiz recommandés",
    "quiz.allInCategory": "Tous les quiz {category}",
    "quiz.otherInCategory": "{count} autre(s) quiz dans cette catégorie",
    "quiz.discoverCategory": "Découvre tous les quiz de cette catégorie",
    "quiz.correct": "Correct !",
    "quiz.wrong": "Faux !",
    "quiz.explanation": "Explication",

    "cat.quizzes": "quiz",
    "cat.subcategories": "Sous-catégories",
    "cat.allQuizzes": "Tous les quiz",
    "cat.featured": "À la une",
    "cat.filter": "Filtrer",
    "cat.sort.popular": "Plus populaires",
    "cat.sort.new": "Plus récents",
    "cat.sort.all": "Tous",
    "cat.explore": "Explorer les catégories",
    "cat.noQuiz": "Pas encore de quiz dans cette catégorie.",

    "difficulty.easy": "Facile",
    "difficulty.medium": "Moyen",
    "difficulty.hard": "Difficile",

    "gameType.qcm": "QCM Classique",
    "gameType.vrai-faux": "Vrai ou Faux",
    "gameType.chrono": "Chrono Challenge",
    "gameType.estimation": "Estimation",
    "gameType.duel": "Duel : qui est le plus ?",
    "gameType.ordre": "Ordre chronologique",

    "sitemap.title": "Plan du site",
    "sitemap.mainPages": "Pages principales",
    "sitemap.allCategories": "Toutes les catégories",

    "footer.about": "À propos",
    "footer.legal": "Mentions légales",
    "footer.privacy": "Confidentialité",
    "footer.terms": "CGU",
    "footer.sitemap": "Plan du site",
    "footer.description": "Des quiz de culture générale gratuits, avec une explication sous chaque réponse. Aucun compte nécessaire pour jouer.",
    "footer.quizzesAndGames": "Quiz et Jeux",
    "footer.allQuizzes": "Tous les quiz",
    "footer.createQuiz": "Créer un quiz",
    "footer.community": "Quiz de la communauté",
    "footer.information": "Informations",
    "footer.contactUs": "Nous contacter",
    "footer.allRightsReserved": "Tous droits réservés.",

    "breadcrumb.home": "Accueil",
    "games.hubName": "Jeux",
    "games.hubHeading": "Des minis-jeux à lancer tout de suite",
    "games.hubIntro": "Des jeux courts qui démarrent en un clic, sans compte et sans rien à préparer. Ils tirent leurs manches au hasard, du coup on peut revenir sur le même et ne pas rejouer la même chose.",
    "games.hubMetaTitle": "Minis-jeux gratuits à jouer en ligne | WizyQuiz",
    "games.hubMetaDescription": "Des jeux de culture générale qui démarrent en un clic, sans compte. Plus haut ou plus bas sur les pays, les sommets, les fleuves et les films.",
    "games.howToPlay": "Comment on joue",
    "games.faq": "Questions fréquentes",
    "games.moreTitle": "Les autres jeux",
    "games.moreIntro": "Tous les jeux d'ici démarrent en un clic et ne vous demandent rien.",
    "games.seeAll": "Voir tous les jeux",
    "games.vsQuizTitle": "Un jeu et un quiz, ce n'est pas la même chose",
    "games.vsQuizP1": "Un quiz a vingt questions, un score et une fin : on le joue, on a son résultat, et voilà. Un jeu tire ses manches au hasard et ne s'arrête que quand on se trompe, du coup le même jeu donne autre chose à chaque fois.",
    "games.vsQuizP2": "Les deux vont ensemble. Le quiz, c'est celui qu'on envoie à un ami pour comparer les scores sur un sujet qu'on connaît tous les deux. Le jeu, c'est celui qu'on ouvre en attendant le bus.",
    "games.seeQuizzes": "Voir les quiz",

    // Auth
    "auth.login": "Se connecter",
    "auth.signup": "S'inscrire",
    "auth.email": "Adresse e-mail",
    "auth.password": "Mot de passe",
    "auth.confirmPassword": "Confirmer le mot de passe",
    "auth.username": "Nom d'utilisateur",
    "auth.loginCta": "Se connecter",
    "auth.signupCta": "Créer mon compte",
    "auth.noAccount": "Pas encore de compte ?",
    "auth.hasAccount": "Déjà un compte ?",
    "auth.loginTitle": "Se connecter à WizyQuiz | Quiz gratuits en ligne",
    "auth.signupTitle": "Inscription gratuite | WizyQuiz",
    "auth.loginDescription": "Connectez-vous à votre compte WizyQuiz pour sauvegarder vos scores, suivre votre progression et défier vos amis.",
    "auth.signupDescription": "Créez votre compte WizyQuiz gratuitement et commencez à suivre vos scores, gagner des XP et vous mesurer à vos amis.",

    // Ranking
    "ranking.title": "Classement",
    "ranking.subtitle": "Grimpez dans le classement en gagnant des XP grâce aux quiz. Plus vous jouez, plus vous montez.",
    "ranking.rank": "Rang",
    "ranking.player": "Joueur",
    "ranking.level": "Niveau",
    "ranking.xp": "XP",
    "ranking.quizzes": "Quiz",
    "ranking.allTime": "Depuis toujours",
    "ranking.thisWeek": "Cette semaine",
    "ranking.thisMonth": "Ce mois-ci",
    "ranking.metaTitle": "Classement des meilleurs joueurs | WizyQuiz",
    "ranking.metaDescription": "Consultez le classement WizyQuiz. Affrontez les autres joueurs, gagnez des XP en complétant des quiz et grimpez dans le classement.",

    "misc.free": "Gratuit",
    "misc.seeMore": "Voir plus",
    "misc.language": "Langue",
  },

  es: {
    "nav.home": "Inicio",
    "nav.categories": "Categorías",
    "nav.create": "Crear un quiz",
    "nav.profile": "Mi Perfil",
    "nav.login": "Iniciar sesión",
    "nav.signup": "Registrarse",
    "auth.logout": "Cerrar sesión",
    "nav.account": "Mi cuenta",
    "nav.search": "Buscar un quiz, una categoría...",
    "nav.ranking": "Clasificación",

    "home.hero.title": "La plataforma de quiz n°1",
    "home.hero.subtitle": "¡Pon a prueba tus conocimientos con nuestros quiz divertidos e interactivos!",
    "home.hero.cta": "Empezar un quiz",
    "home.hero.create": "Crear un quiz",
    "home.stats.quizzes": "quizzes",
    "home.stats.questions": "preguntas",
    "home.stats.plays": "partidas jugadas",
    "home.popular": "Los más populares",
    "home.new": "Novedades",
    "home.categories": "Todas las categorías",
    "home.seeAll": "Ver todo",
    "home.trending": "Tendencias",
    "home.gameTypes": "Modos de juego",
    "home.questionOfDay": "Pregunta del día",
    "home.faq.title": "Preguntas frecuentes",
    "home.member.title": "Hazte miembro",
    "home.member.cta": "Crear mi cuenta",
    "home.member.free": "Gratis, sin compromiso",
    "home.member.scores": "Guarda tus puntuaciones",
    "home.member.xp": "Gana puntos XP",
    "home.member.compare": "Compárate con otros",
    "home.member.text": "Una cuenta guarda tus puntuaciones y tu XP, y te permite compararte con tus amigos.",

    "quiz.questions": "preguntas",
    "quiz.question": "Pregunta",
    "quiz.plays": "partidas jugadas",
    "quiz.difficulty": "Dificultad",
    "quiz.start": "Empezar quiz",
    "quiz.next": "Siguiente",
    "quiz.finish": "Ver resultados",
    "quiz.score": "Tu puntuación",
    "quiz.perfect": "¡Puntuación perfecta!",
    "quiz.great": "¡Genial!",
    "quiz.good": "¡Bien hecho!",
    "quiz.retry": "Reintentar",
    "quiz.share": "Compartir",
    "quiz.recommended": "Quizzes recomendados",
    "quiz.allInCategory": "Todos los quiz de {category}",
    "quiz.otherInCategory": "{count} otro(s) quiz en esta categoría",
    "quiz.discoverCategory": "Descubre todos los quizzes de esta categoría",
    "quiz.correct": "¡Correcto!",
    "quiz.wrong": "¡Incorrecto!",
    "quiz.explanation": "Explicación",

    "cat.quizzes": "quiz",
    "cat.subcategories": "Subcategorías",
    "cat.allQuizzes": "Todos los quiz",
    "cat.featured": "Destacado",
    "cat.filter": "Filtrar",
    "cat.sort.popular": "Más populares",
    "cat.sort.new": "Más recientes",
    "cat.sort.all": "Todos",
    "cat.explore": "Explorar categorías",
    "cat.noQuiz": "Aún no hay quizzes en esta categoría.",

    "difficulty.easy": "Fácil",
    "difficulty.medium": "Medio",
    "difficulty.hard": "Difícil",

    "gameType.qcm": "Opción Múltiple",
    "gameType.vrai-faux": "Verdadero o Falso",
    "gameType.chrono": "Desafío Crono",
    "gameType.estimation": "Estimación",
    "gameType.duel": "Duelo: ¿quién tiene más?",
    "gameType.ordre": "Orden cronológico",

    "sitemap.title": "Mapa del sitio",
    "sitemap.mainPages": "Páginas principales",
    "sitemap.allCategories": "Todas las categorías",

    "footer.about": "Acerca de",
    "footer.legal": "Aviso legal",
    "footer.privacy": "Privacidad",
    "footer.terms": "Términos",
    "footer.sitemap": "Mapa del sitio",
    "footer.description": "Quizzes de cultura general gratis, con una explicación debajo de cada respuesta. No hace falta cuenta para jugar.",
    "footer.quizzesAndGames": "Quizzes y Juegos",
    "footer.allQuizzes": "Todos los quizzes",
    "footer.createQuiz": "Crear un quiz",
    "footer.community": "Quizzes de la comunidad",
    "footer.information": "Información",
    "footer.contactUs": "Contáctanos",
    "footer.allRightsReserved": "Todos los derechos reservados.",

    "breadcrumb.home": "Inicio",
    "games.hubName": "Juegos",
    "games.hubHeading": "Minijuegos para empezar ahora mismo",
    "games.hubIntro": "Juegos cortos que arrancan con un toque, sin cuenta y sin nada que preparar. Sacan sus rondas al azar, así que puedes volver al mismo y no jugar lo mismo dos veces.",
    "games.hubMetaTitle": "Minijuegos gratis para jugar online | WizyQuiz",
    "games.hubMetaDescription": "Juegos de cultura general que arrancan con un toque, sin cuenta. Mayor o menor sobre países, cumbres, ríos y películas, con racha que batir.",
    "games.howToPlay": "Cómo se juega",
    "games.faq": "Preguntas frecuentes",
    "games.moreTitle": "Los otros juegos",
    "games.moreIntro": "Todos los juegos de aquí arrancan con un toque y no te piden nada.",
    "games.seeAll": "Ver todos los juegos",
    "games.vsQuizTitle": "Un juego y un quiz no son lo mismo",
    "games.vsQuizP1": "Un quiz tiene veinte preguntas, una puntuación y un final: lo juegas, tienes tu resultado, y ya está. Un juego saca sus rondas al azar y solo se para cuando fallas, así que el mismo juego te da otra cosa cada vez.",
    "games.vsQuizP2": "Los dos van juntos. El quiz es el que le mandas a un amigo para comparar puntuaciones sobre un tema que os sabéis los dos. El juego es el que abres esperando el autobús.",
    "games.seeQuizzes": "Ver los quizzes",

    // Auth
    "auth.login": "Iniciar sesión",
    "auth.signup": "Registrarse",
    "auth.email": "Correo electrónico",
    "auth.password": "Contraseña",
    "auth.confirmPassword": "Confirmar contraseña",
    "auth.username": "Nombre de usuario",
    "auth.loginCta": "Iniciar sesión",
    "auth.signupCta": "Crear mi cuenta",
    "auth.noAccount": "¿Aún no tienes cuenta?",
    "auth.hasAccount": "¿Ya tienes una cuenta?",
    "auth.loginTitle": "Iniciar sesión en WizyQuiz | Quizzes gratuitos",
    "auth.signupTitle": "Regístrate gratis | WizyQuiz, quizzes online",
    "auth.loginDescription": "Inicia sesión en tu cuenta WizyQuiz para guardar tus puntuaciones, seguir tu progreso y desafiar a tus amigos.",
    "auth.signupDescription": "Crea tu cuenta WizyQuiz gratis y empieza a seguir tus puntuaciones, ganar XP y competir con tus amigos.",

    // Ranking
    "ranking.title": "Clasificación",
    "ranking.subtitle": "Sube en la clasificación ganando XP con los quiz. Cuanto más juegues, más alto llegaras.",
    "ranking.rank": "Rango",
    "ranking.player": "Jugador",
    "ranking.level": "Nivel",
    "ranking.xp": "XP",
    "ranking.quizzes": "Quizzes",
    "ranking.allTime": "Desde siempre",
    "ranking.thisWeek": "Esta semana",
    "ranking.thisMonth": "Este mes",
    "ranking.metaTitle": "Clasificación de los mejores jugadores | WizyQuiz",
    "ranking.metaDescription": "Consulta la clasificación de WizyQuiz. Compite con otros jugadores, gana XP completando quizzes y sube en el ranking.",

    "misc.free": "Gratis",
    "misc.seeMore": "Ver más",
    "misc.language": "Idioma",
  },
} as const;

export type UIKey = keyof (typeof ui)["en"];

export function t(locale: Locale, key: UIKey, params?: Record<string, string | number>): string {
  let str = (ui[locale]?.[key] ?? ui.en[key] ?? key) as string;
  if (params) {
    for (const [k, v] of Object.entries(params)) {
      str = str.replace(`{${k}}`, String(v));
    }
  }
  return str;
}
